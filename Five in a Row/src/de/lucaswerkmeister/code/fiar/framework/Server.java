 ...
 * The game server.
 * <p>
 * The server handles all game logic. It decides whether a move is legal, keeps
 * track of the game, and notifies the {@link Client}s about any events.
 * <p>
 * All methods of the server are meant to be called by a client; the first
 * argument of each method call should always be the calling instance.
 * <p>
 * The server's behavior when encountering an unknown client as requester is
...
public abstract class Server {
	 ...
	 * <p>
	 * Phases represent states of the game like "initialization" or
	 * "placing stones" (for a complete list, see below). Each phase can have
	 * sub-phases; the value <code>{1,1,3,8}</code> therefore represents
	 * "Phase 1.1.3.8" or "Sub-Phase 8 of Sub-Phase 3 of Sub-Phase 1 of Phase 1"
	 * <p>
	 ...
	 * indicated that they are happy with the current distribution of Joker
	 * fields.</li>
	 * </ol>
	 * <li>Game running</li>
	 * <ol start="0">
	 * <li>Server is calculating. Note: As server calculations typically run
	 * very short, a server implementation may choose to never return this
	 * status and instead wait until the server is done calculating.</li>
	 * <li>One specific player may place a stone. The sub-phase of this phase
	 * gives the player's ID.</li>
	 * </ol>
	 * <li>Game ended
	 * <ol start="0">
	 ...
	 * <li>Quit: The game ended because one player quit the game. Note: One
	 * player quitting does not necessarily end the game - the server can, for
	 * example, choose to continue the game, and if only one player is left,
	 * declare this player the winner.</li>
	 ...
	 * is provided for compatibility between different server and client
	 * versions.
	 * <p>
	 ...
	 * <p>
	 * For more detailed informations about which actions a player can currently
	 * perform, use {@link #getAllowedActions(Client, Player)}.
	 * 
	 * @param requester
	 *            The requesting client.
	 * @param p
	 ...
	 * <p>
	 * Note that the server's internal board can not be manipulated by
	 * manipulating the {@link Board} instance retrieved by this method: Either
	 * the server returns a copy of its internal Board instance, or its internal
	 * board representation doesn't use the class Board altogether.
	 * <p>
	 * If the board has not yet been decided on, this method returns
	 * <code>null</code>.
	 * 
	 * @param requester
	 *            The requesting client.
